...
//	rfb::Region2D clipregion;
	...
	while (1)
	{
		// Block waiting for an update to send
		{
			#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"vncClientUpdateThread: GetUpdateLock1 \n");
			...
			#ifdef _DEBUG
//					char			szText[256];
					sprintf(szText,"vncClientUpdateThread: GetUpdateLock2 \n");
					OutputDebugString(szText);
			#endif
			...
			(m_client->m_encodemgr.m_scrinfo.format.bitsPerPixel == m_client->m_encodemgr.m_buffer->m_scrinfo.format.bitsPerPixel))
		{
			#ifdef _DEBUG
					char			szText[256];
					sprintf(szText," Client: SendUpdate \n");
					OutputDebugString(szText);
			#endif
			...
			if (m_client->SendUpdate(update)) {
				updates_sent++;
				m_client->m_incr_rgn.clear();//clipregion.clear();
			}
		}
		else
		{
...
vncClientThread::InitVersion()
{
	...
	if (strcmp(m_client->ProtocolVersionMsg,"0.0.0.0")==NULL)
	{
		...
		sprintf((char *)protocolMsg,
			...
			rfbProtocolMinorVersion +10+ (m_server->MSLogonRequired() ? 0 : 2)); // 4: mslogon+FT,
																			// 6: VNClogon+FT
...
#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"Client: msg.type %i\n",msg.type);
					OutputDebugString(szText);
#endif
...
#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"Client: rfbFramebufferUpdateRequest \n");
					OutputDebugString(szText);
#endif
			// Read the rest of the message:
...
#ifdef _DEBUG
					//char			szText[256];
					sprintf(szText," Client:TriggerUpdateThread \n");
					OutputDebugString(szText);
#endif
				}
			}
